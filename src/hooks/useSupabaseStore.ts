...
export const useSupabaseStore = () => {
  ...
  const loadUserData = useCallback(async () => {
    ...
    try {
      ...
      // Create a set of all dates that need logs (existing logs + dates with tasks)
      const datesWithTasks = new Set([
        ...logsData.map(log => log.date),
        ...Object.keys(groupedTasks)
      ]);

      // Create logs array with both existing logs and virtual logs for dates with tasks
      const allLogs = Array.from(datesWithTasks).map(date => {
        const existingLog = logsData.find(log => log.date === date);
        if (existingLog) {
          return {
            ...existingLog,
